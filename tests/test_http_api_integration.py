...
def test_health_ready_and_error_contracts(api_runtime):
    ...
    assert ready["semantic_classifier"]["integrity_verified"] is True
    assert ready["semantic_classifier"]["classes_valid"] is True
...
def test_concurrent_chat_and_stats_requests(api_runtime):
    ...
    with ThreadPoolExecutor(max_workers=12) as executor:
        results = list(executor.map(send, range(60)))

    assert all(status == 200 for status, _ in results)
    assert all(payload["allowed"] is True for _, payload in results)
    assert pipeline.logger.stats()["total_events"] == 60
