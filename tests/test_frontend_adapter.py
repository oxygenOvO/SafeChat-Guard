...
def make_adapter() -> FrontendPipelineAdapter:
    pipeline = SafeChatPipeline.from_config("config.yaml")
    return FrontendPipelineAdapter(pipeline)


def test_adapter_uses_real_pipeline_for_obfuscated_ad():
...
def test_adapter_uses_real_pipeline_for_homophone_contact():
...
def test_adapter_can_check_overridden_model_output():
...
def test_adapter_exposes_real_rule_configuration():
