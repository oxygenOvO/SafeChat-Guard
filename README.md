...
当前版本已经形成可运行、可联调、可复现评测的工程原型：前端使用 Streamlit 展示完整安全链路，后端通过 `SafeChatPipeline` 串联中文对抗归一化、规则过滤、轻量语义分类、分级处理、Mock 大模型调用、输出复检和脱敏日志统计。项目同时提供数据审计、对抗评测集构建、独立人工评测和语义基线 V2 流程，便于四个分工继续并行优化并接入真实 Qwen/ChatGLM 等大模型服务。
...
- 工程安全：限制 API 请求体大小和输入类型，不向前端返回被拦截的原始模型输出，日志落盘前统一脱敏。
...
当前自动化测试结果（2026-07-18）：

```text
104 passed
```

测试覆盖后端主流程、输出校验、归一化、前端适配、配置阈值、规则真实命中、模型降级、日志脱敏、数据审计、独立评测流程和 `/api/detect` 返回结构。
...
- 后端日志默认保存在 `data/logs/events.jsonl`，敏感字段写入前会脱敏，该文件不会提交到 GitHub。
...
- 当前完整自动化测试为 104 项，并配置 GitHub Actions 自动运行测试与 Python 编译检查。
...
2. 完成 200 条独立评测候选的双人复核，生成不含数据泄漏的正式 gold 集。
3. 扩充有来源依据的真实攻击样本和高风险短句，继续核验中文对抗映射及误报边界。
4. 在独立验证集上重新校准语义分类阈值，报告分类指标、误报率和对抗鲁棒性。
5. 将输出侧部分硬编码规则迁移到配置文件。
6. 单独完成前端参赛级优化，并整理项目报告、测试截图、答辩 PPT 和最终提交材料。
